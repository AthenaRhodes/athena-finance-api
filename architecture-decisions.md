...
| 17 | **Forex rates via Frankfurter.app, not Finnhub** | Finnhub free tier returns 403 for forex quotes. Frankfurter.app is free, requires no API key, publishes official ECB daily rates, and supports historical data. Suitable for EUR/USD, GBP/USD etc. Limitation: ECB rates only — exotic pairs may not be available. |
| 18 | **Two-provider strategy: Finnhub (watchlist) + Polygon.io (universe)** | Finnhub free tier is limited to 60 calls/min — sufficient for watchlist-level data (quote, profile, metrics per symbol). Polygon.io free tier provides a bulk snapshot endpoint returning all ~18k US stocks in a single call, making full-universe daily EOD loads feasible. Finnhub handles live + per-security detail; Polygon handles universe-scale batch operations. |
| 19 | **Universe table separate from Watchlist** | `Securities` grows to cover the full Polygon universe. `WatchlistItems` remains a user-curated subset. EOD prices stored for all securities (not just watched ones), enabling the user to search, browse and add from the full universe. |
